...
class HillClimb{
    ...
    public: 
        HillClimb(int t,int z, int l, vector<vector<int> > Times, vector<vector<int> > Ns){
            this->t = t*60;
            this->z = z;
            this->l = l;
            this->T = Times;
...
stateNode* HillClimb::randomZoneAssign(){
    ...
    int best_cost_till_now=INT_MAX;
    ...
    for(int i=0;i<10;i++){
        ...
        if(node->heuristicCost<=best_cost_till_now){
            ...
            cout<<"Initial States Sampled:-"<<BestStartNode->heuristicCost<<endl;
        }
    }
