...
stateNode* HillClimb::getBestNeighbour(stateNode* node){
    ...
    while(k<cont_sampling){
                ...
                if(newCost-currCost<0){
                        ...
                        cout<<"newCost-currCost="<<newCost-currCost<<endl;
                        cout<<"Temperature="<<this->Temperature<<endl;
                        double prob_bad_moves=(double)exp(-(double)(newCost-currCost)/((double)this->Temperature));
                        cout<<"prob of taking bad move "<<prob_bad_moves<<endl;
                        cout<<"prob "<<prob<<endl;
...
void HillClimb::startSearch(){
    ...
    while(this->time_limit>duration_cast<microseconds>(high_resolution_clock::now()- start).count()){
        ...
        while(c<step_limit && this->time_limit>duration_cast<microseconds>(high_resolution_clock::now()- start).count()){
            ...
            this->Temperature=(60*1000000*l)/time;
            // this->Temperature=l*l*l*exp(-((time)*(time))/ (2* (0.2)*(0.2)));
            //this->Temperature=10*(-time+time_limit);
